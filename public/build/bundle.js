...
var app = (function () {
    ...
    var messages_5 = messages.MapRow;
    ...
    function create_if_block(ctx) {
    	let t_value = /*cell*/ ctx[13].slice(1) + "";
    	let t;
    ...
    // (159:3) {#each row.row as cell}
    function create_each_block_1(ctx) {
    	let td;
    	let if_block = /*cell*/ ctx[13] && create_if_block(ctx);

    	const block = {
    		c: function create() {
    			td = element("td");
    			if (if_block) if_block.c();
    			set_style(td, "background-color", cellToColor(/*cell*/ ctx[13].charAt(0)));
    			attr_dev(td, "class", "svelte-18ahk4x");
    			add_location(td, file, 159, 4, 8050);
    		},
    		m: function mount(target, anchor) {
    		...
    		p: function update(ctx, dirty) {
    			if (/*cell*/ ctx[13]) if_block.p(ctx, dirty);
    		},
    		d: function destroy(detaching) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(159:3) {#each row.row as cell}",
    		ctx
    	});
    ...
    // (157:1) {#each decoded.map as row}
    function create_each_block(ctx) {
    	let tr;
    	let t;
    	let each_value_1 = /*row*/ ctx[10].row;
    	let each_blocks = [];
    	...
    	const block = {
    		c: function create() {
    			...
    			t = space();
    			attr_dev(tr, "class", "svelte-18ahk4x");
    			add_location(tr, file, 157, 2, 8014);
    		},
    		m: function mount(target, anchor) {
    		...
    		p: function update(ctx, dirty) {
    			if (dirty & /*cellToColor, decoded*/ 1) {
    				each_value_1 = /*row*/ ctx[10].row;
    				let i;
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(157:1) {#each decoded.map as row}",
    		ctx
    	});
    ...
    function create_fragment(ctx) {
    	let table;
    	let dispose;
    	let each_value = /*decoded*/ ctx[0].map;
    	let each_blocks = [];
    	...
    	const block = {
    		c: function create() {
    			...
    			attr_dev(table, "class", "svelte-18ahk4x");
    			add_location(table, file, 155, 0, 7976);
    		},
    		...
    		p: function update(ctx, [dirty]) {
    			if (dirty & /*decoded, cellToColor*/ 1) {
    				each_value = /*decoded*/ ctx[0].map;
    				let i;
    ...
    function instance($$self) {
    	...
    	let encodedMap = Array.from(map, row => messages_5.create({ row }));

    	let message = messages_1.create({
    		map: encodedMap,
    		players: ["1", "2"],
    		order: ["he"]
    	});

    	let buffer = messages_1.encode(message).finish(); // send to the 
    	let decoded = messages_1.decode(buffer);
    	...
    	socket.on("tick", data => {
    		if (data.map) ; // data.map.forEach(row => { // 	console.log(row);
    	}); // }); // map = data.map;
    	...
    	$$self.$inject_state = $$props => {
    		...
    		if ("encodedMap" in $$props) encodedMap = $$props.encodedMap;
    		if ("message" in $$props) message = $$props.message;
    		if ("buffer" in $$props) buffer = $$props.buffer;
    		if ("decoded" in $$props) $$invalidate(0, decoded = $$props.decoded);
    		if ("myMap" in $$props) myMap = $$props.myMap;
    		if ("map2" in $$props) map2 = $$props.map2;
    	};

    	return [decoded, handleKeydown];
    }
